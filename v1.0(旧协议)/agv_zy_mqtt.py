...
class MoveBaseDoor():
    def __init__(self):
        ...
        self.action0, self.action1, self.action2, self.action3 = True, False, False, False
	# 初始化数据
	...
	# 接收到上料完成进行搬运任务
        elif self.nav_id_result == 1 or ((self.sub_name == "AD") and (self.sub_dir == "AGV") and (self.sub_action == 0) and self.action0):
                self.AD_flag = False
                if self.and_id:
                ...
                if self.i == 2:
                    self.nav_id_result = -1
                    self.i, self.action1, self.action2 = 0, False, True
	...
	# 接收到下料完成回到原点
        elif self.nav_id_result == 3 or ((self.sub_name == 'TD') and (self.sub_dir == "AGV") and (self.sub_action == 0) and self.action2):
                self.TD_flag = False
                if self.td_id:
